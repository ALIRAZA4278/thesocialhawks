            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
                {service.pricing.map((pkg, index) => (
                  <motion.div 
                    key={index}
                    className={`relative bg-white rounded-2xl shadow-lg border-2 p-8 ${pkg.popular ? 'border-primary transform scale-105' : 'border-gray-200'}`}
                    initial={{ opacity: 0, y: 30 }}
                    animate={isProcessInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }}
                  ...
                  >
                    {pkg.popular && (
                      <motion.div 
                        className="absolute -top-4 left-1/2 transform -translate-x-1/2 bg-primary text-white px-6 py-2 rounded-full text-sm font-semibold"
                        initial={{ scale: 0 }}
                    ...
                    <div className="text-center mb-8">
                      <h4 className="text-2xl font-bold text-gray-900 mb-4">{pkg.package}</h4>
                      <div className="mb-2">
                    ...
                    <ul className="space-y-4 mb-8">
                      {pkg.features.map((feature, featureIndex) => (
                        <motion.li 
                          ...
                          initial={{ opacity: 0, x: -20 }}
                          animate={isProcessInView ? { opacity: 1, x: 0 } : { opacity: 0, x: -20 }}
                          transition={{ duration: 0.6, delay: 1.3 + index * 0.2 + featureIndex * 0.1 }}
                        >
                          <motion.div 
                            className="w-5 h-5 bg-primary text-white rounded-full flex items-center justify-center text-xs flex-shrink-0 mt-0.5"
                    ...
                    <motion.button 
                      className={`w-full py-4 rounded-xl font-semibold transition-all ${pkg.popular ? 'bg-primary text-white hover:bg-primary-hover' : 'bg-gray-900 text-white hover:bg-gray-800'}`}
